...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional grid for tiling inference, e.g., '3x3' (rows x cols). When set, each view is tiled and tile logits are averaged.",
    )
    parser.add_argument(
        "--tile-overlap",
        type=float,
        default=0.125,
        help="Tile overlap ratio per axis (0.0~0.4 recommended).",
    )
    parser.add_argument(
        "--save-tile-probs",
        action="store_true",
        help="When tiling is enabled, save per-tile averaged probabilities to outputs/logits_tiles/*.npz",
    )
...
def main() -> None:
    ...
    if args.tile_grid:
        try:
            parts = args.tile_grid.lower().split('x')
            tile_rows, tile_cols = int(parts[0]), int(parts[1])
        except Exception:
            raise ValueError("--tile-grid must be like '3x3'")
        if tile_rows < 1 or tile_cols < 1:
            raise ValueError("--tile-grid values must be >=1")
        overlap = max(0.0, float(args.tile_overlap))
        overlap = min(overlap, 0.4)
    else:
        overlap = 0.0

    def run_model_on(xt: torch.Tensor) -> torch.Tensor:
    ...
    def run_tiled(aug: torch.Tensor):
        """Run model on tiles.
        Returns:
          mean_logits: (B,num_classes) averaged over tiles
          tile_probs: (B, T, num_classes) softmax probs per tile (T=tile_rows*tile_cols)
        If tiling disabled, returns (logits, None).
        """
        # aug: (B,C,H,W) normalized
        if not (tile_rows and tile_cols):
            logits = run_model_on(aug)
            return logits, None
        B, C, H, W = aug.shape
        out_logits = []
        out_tile_probs = []
        # compute tile size/stride
        tile_h = int((H + tile_rows - 1) // tile_rows)
        tile_w = int((W + tile_cols - 1) // tile_cols)
        ovh = int(round(tile_h * overlap))
        ovw = int(round(tile_w * overlap))
        stride_h = max(1, tile_h - ovh)
        stride_w = max(1, tile_w - ovw)
        # generate top-lefts ensuring exactly tile_rows x tile_cols tiles
        ys = [min(i*stride_h, max(0, H - tile_h)) for i in range(tile_rows)]
        xs = [min(j*stride_w, max(0, W - tile_w)) for j in range(tile_cols)]
        for i in range(B):
            tiles = []
            for y in ys:
                for x in xs:
                    crop = aug[i:i+1, :, y:y+tile_h, x:x+tile_w]
                    # resize back to (H,W) expected by model
                    crop_resized = Fnn.interpolate(crop, size=(H, W), mode='bilinear', align_corners=False)
                    tiles.append(crop_resized)
            xt = torch.cat(tiles, dim=0)  # (T,C,H,W)
            logits = run_model_on(xt)      # (T,num_classes)
            probs = torch.softmax(logits, dim=1)  # (T,num_classes)
            logits_mean = logits.mean(dim=0, keepdim=True)  # (1,num_classes)
            out_logits.append(logits_mean)
            out_tile_probs.append(probs.unsqueeze(0))  # (1,T,C)
        return torch.cat(out_logits, dim=0), torch.cat(out_tile_probs, dim=0)  # (B,C) and (B,T,C)
    if use_denoise:
        import cv2
    ...
    start_ts = _time.time()
    log_every = max(1, int(total_batches * 0.02)) if total_batches else 50

    with torch.no_grad():
        save_tile = bool(tile_rows and tile_cols and args.tile_grid and args.save_tile_probs)
        tile_probs_accum_list = []
        for images, ids in test_loader:
            ...
            for angle in unique_angles:
                ...
                for hf, vf in flip_options:
                    ...
                    if tile_rows and tile_cols:
                        outputs, tile_probs = run_tiled(aug)
                        if save_tile and tile_probs is not None:
                            tp = tile_probs  # (B,T,C)
                            batch_tile_sum = tp if batch_tile_sum is None else (batch_tile_sum + tp)
                            batch_tile_views += 1
                    else:
                        outputs = run_model_on(aug)
            ...
            if save_tile and batch_tile_sum is not None and batch_tile_views > 0:
                avg_tp = (batch_tile_sum / float(batch_tile_views)).cpu().numpy()  # (B,T,C)
                tile_probs_accum_list.append(avg_tp)
            processed += 1
            if processed % log_every == 0 or (total_batches and processed == total_batches):
                elapsed = _time.time() - start_ts
                if total_batches:
                    print(f"[create_submission] Progress: {processed}/{total_batches} batches ({processed/total_batches*100:.1f}%) elapsed={elapsed/60:.1f}m", flush=True)
                else:
                    print(f"[create_submission] Progress: {processed} batches elapsed={elapsed/60:.1f}m", flush=True)
    ...
    print(f"[create_submission] Saved CSV: {csv_path}", flush=True)

    if args.save_probs:
        probs_dir = Path(cfg["paths"]["output_root"]) / "logits"
        probs_dir.mkdir(parents=True, exist_ok=True)
        np.save(probs_dir / f"{submission_name}.npy", probs_array)
        print(f"[create_submission] Saved logits: {probs_dir / (submission_name + '.npy')}", flush=True)

    # Save averaged tile probabilities across TTA views per image
    if save_tile and tile_probs_accum_list:
        tile_dir = Path(cfg["paths"]["output_root"]) / "logits_tiles"
        tile_dir.mkdir(parents=True, exist_ok=True)
        tile_probs_all = np.concatenate(tile_probs_accum_list, axis=0)  # (N,T,C)
        np.savez_compressed(tile_dir / f"{submission_name}_tiles.npz",
                            probs=tile_probs_all,
                            ids=np.array(image_ids),
                            grid=np.array([tile_rows, tile_cols]),
                            overlap=float(overlap))
        print(f"[create_submission] Saved tile probs: {tile_dir / (submission_name + '_tiles.npz')}", flush=True)
